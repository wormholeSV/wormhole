...
int populateRPCTransactionObject(const uint256& txid, UniValue& txobj, std::string filterAddress, bool extendedDetails, std::string extendedDetailsFilter)
{
    ...
    return populateRPCTransactionObject(*(tx.get()), blockHash, txobj, filterAddress, extendedDetails, extendedDetailsFilter);
}
...
void populateRPCTypeGetBaseProperTy(CMPTransaction& mp_obj, UniValue& txobj, int confirmations){
    ...
    txobj.push_back(Pair("precision", getprecision(OMNI_PROPERTY_WHC)));
...
int populateRPCSendAllSubSends(const uint256& txid, UniValue& subSends)
{
    ...
    for (int subSend = 1; subSend <= numberOfSubSends; subSend++) {
        ...
        subSendObj.push_back(Pair("precision", getprecision(propertyId)));
