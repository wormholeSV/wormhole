...
std::string ParseAddress(const UniValue& value)
{
    ...
    if (addr == comAddr){
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address. Note: use cashAddress");
    }

    return EncodeCashAddr(addr, param);
}
...
CTransaction ParseTransaction(const UniValue& value)
{
   CMutableTransaction	tx;
    if (value.isNull() || value.get_str().empty()) {
        //return CTransaction(const_cast<const CMutableTransaction>(tx));
...
std::vector<PrevTxsEntry> ParsePrevTxs(const UniValue& value)
{
    ...
    for (size_t i = 0; i < prevTxs.size(); ++i) {
        ...
        int64_t nValue = AmountFromValue(outputValue).GetSatoshis();
